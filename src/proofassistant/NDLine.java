...
//        System.out.println("NDLines says " + Globals.terms.getListOfUsedTerms().contains("s"));
     ...
     * @param macro The TeX code of the line to create
     */
    public NDLine(String macro) { // Creates a standard line
        id = System.currentTimeMillis();
        line = processMacro(macro);
        mainOp = findMainOp(line);
        firstArg = findFirstArg(line);
     ...
     */
    public NDLine(String macro, boolean incrementLine) { // Creates a standard line, but doesn't increment line num
        id = System.currentTimeMillis();
        line = processMacro(macro);
        mainOp = findMainOp(line);
        firstArg = findFirstArg(line);
...
//        System.out.println(line);
    ...
    private String findNonUniMainOp(String line) { // Returns the first non universal main operator, or qa if there is no other operator
        if (findMainOp(line).equals("qa")) {
            if (findMainOp(findSecondArg(line)).equals("")) {
                ...
                return findNonUniMainOp(findSecondArg(line));
            }
        } else {
            return findMainOp(line);
        }
    ...
    private String findNonUniFirstArgRegEx(String line) { // Returns the first non universal argument of a line as a regular expression, or the line as a regex if no first argument
//        System.out.println();
...
//        System.out.println(line);
    ...
    private String findNonUniSecondArgRegEx(String line) { // Returns the second argument of line as a regex, or "" if no second argument
//        System.out.println();
...
//        System.out.println();
    ...
    private String findNonUniRegEx(String line) {
...
//        System.out.println();
    ...
    private String findRegEx(String variable) {
        String regEx = line.replace("\\", "\\\\"); // Replace \ with \\ to make regex feel happy
...
//            }
    ...
    public String getNonUniMainOp() {
        return findNonUniMainOp(line);
    }
    
    public String getNonUniSMainOp() {
        String value = findNonUniMainOp(line);
    ...
    public String parseNonUniMainOp() {
        return convertOp(findNonUniMainOp(line));
    }
    ...
    public String getNonUniFirstArgRegEx() {
        return findNonUniFirstArgRegEx(line);
    }
    
    public String getNonUniSecondArgRegEx() {
        return findNonUniSecondArgRegEx(line);
    }
    
    public String getNonUniRegEx() {
        return findNonUniRegEx(line);
    }
    
    public String getRegEx(String variable) {
        return findRegEx(variable);
    }
